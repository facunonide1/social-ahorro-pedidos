...
### 👉 PRÓXIMA ACCIÓN: **T8 · Alertas** (`/hub/operaciones/alertas`). Cron `alertas-stock` genera los 9 tipos en `alertas_stock`: stock_critico, quiebre_proyectado (dias_stock<=lead), sobrestock, sin_rotacion, stock_fantasma (rotaba >0.5/d + 30d sin vender con stock, o discrepancia import → crítica + notif super+encargado), vencimiento_90/60/30/15 (de lotes). Centro de alertas: tarjetas border-left por severidad + botones de acción por tipo (fantasma→Tarea revisar físico; quiebre→Pedir a sucursal/Reponer; sobrestock→Crear oferta/Redistribuir; sin_rotacion→Ver/Descartar; vencimiento→Ver en vencimientos). Filtros Todas/Críticas/Por sucursal. Badge en sidebar. Config umbrales en /admin/configuracion/inventario. Luego T9 vencimientos acciones, T10 transferencias mueven stock, T11 inventarios 4 sucursales, T12 CrearTareaRapida + recepción→stock + NORA tools + demo 60d + tag v0.8.
