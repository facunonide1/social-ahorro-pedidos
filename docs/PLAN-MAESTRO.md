...
### 👉 PRÓXIMA ACCIÓN: **T9 · Vencimientos (rehacer)** (`/hub/operaciones/vencimientos`). Header con $ en riesgo total. Tabs semáforo Vencidos/≤15/≤30/≤60/≤90. Tabla por lote (producto+SKU+lote, sucursal, días, cantidad, $ riesgo=cantidad×costo) con ACCIÓN SUGERIDA + botón: otra sucursal rota ≥2× → "Transferir a [suc]" (crea transferencia precargada); la cadena no lo vende → "Devolver al proveedor" (crea devolución); intermedio → "Ofertar -X%" (descuento óptimo). Botones "Tarea: retirar vencidos" + "Dar de baja vencidos" (movimiento baja_vencimiento). Export Excel. Luego T10 transferencias mueven stock, T11 inventarios 4 sucursales, T12 CrearTareaRapida + recepción→stock + NORA tools + demo 60d + tag v0.8.
