...
## 👉 PRÓXIMA ACCIÓN
**T3 · Reorganización del sidebar (8 pilares).** Reescribir
`lib/constants/navegacion.ts` a la estructura por pilares (MISSION CONTROL,
OPERACIÓN, FINANZAS, COMPRAS, EQUIPO, CLIENTES, INTELIGENCIA, ADMINISTRACIÓN
solo super_admin), sin duplicados (Facturas solo en Finanzas), eliminar
"Admin (nuevo)", CRM Pedidos como ítem normal en OPERACIÓN. Render por grupos
en el sidebar (`components/layout/*` para `/admin`; decidir si el sidebar nuevo
unifica `/hub` o se mantiene el shell legacy redirigiendo). Item activo:
`bg-nora-bg` + `text-primary`.
