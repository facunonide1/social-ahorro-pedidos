...
export default async function CalendarioOfertasPage() {
  ...
  const [{ data: ofertas }, { data: camps }] = await Promise.all([
    sb.from('ofertas').select('id, codigo, nombre, tipo, estado, fecha_inicio, fecha_fin, campania_id').not('estado', 'in', '("rechazada")').limit(1000),
  ...
  const eventos: EventoOferta[] = ((ofertas ?? []) as any[])
    ...
    .map((o) => ({ id: o.id, nombre: o.nombre, tipo: o.tipo, estado: o.estado, fecha_inicio: o.fecha_inicio, fecha_fin: o.fecha_fin }))
  ...
  return (
    <>
      ...
      <div className="space-y-4 p-4 md:p-6">
        <CalendarioOfertasClient eventos={eventos} campanias={campanias} hoy={new Date().toISOString().slice(0, 10)} />
      </div>
    </>
  )
}
