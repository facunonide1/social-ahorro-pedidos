...
export default async function ClientesPage({
  ...
  let query = sb
    .from('customers')
    .select('*, orders_count:orders(count)')
  ...
  const customers: CustomerRow[] = (raw ?? []).map((r: any) => ({
    ...r,
    orders_count: Array.isArray(r.orders_count) ? (r.orders_count[0]?.count ?? 0) : 0,
  }))
        ...
        {customers.map(c => (
          <Link key={c.id} href={`/clientes/${c.id}`}
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: '#2a2a2a' }}>{c.name || '(sin nombre)'}</div>
              ...
              {c.tags.length > 0 && (
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 6 }}>
                  {c.tags.map(t => (
                    <span key={t} style={{ fontSize: 10, fontWeight: 700, color: '#726DFF', background: '#eeedff', border: '0.5px solid #d9d6ff', padding: '1px 6px', borderRadius: 999, letterSpacing: '0.3px', textTransform: 'uppercase' }}>
                      {t}
                    </span>
                  ))}
                </div>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#2a2a2a', letterSpacing: '-0.5px' }}>
                {c.orders_count}
              </div>
